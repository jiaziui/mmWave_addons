...
# 开发需求
帮我完成 设备管理界面和功能（开发完部分功能）。详细描述内容如下，其中一些内容你不确定最好跟我确定，觉得不合理的给我解决方案并且跟我确认方案。工程在这个路径下D:\User\lj\DFRobot\Project\module\C4004\C4004\05_Software\home_assistant\mmWave_addons

## 详细需求描述：
### 界面三大部分（设备总览、设备管理、区域管理）

1.设备总览（已经开发完成，不用管，不用开发）
    ...
    1.1设备详情展示主界面（风格参考：05_Software\home_assistant\mmWave_addons\resource\设备详情展示界面.png）
...
2.设备管理（已经开发完成，不用管，不用开发）
    2.1. 目前已经完成扫描设备，也可以扫描出设备，但是，初始化绑定功能并没有体现出来,功能已经写了,现在我需要在后端中去存储绑定的数据,存储绑定数据到devices（后端新创建一个文件devices.json（放在这个路径下/homeassistant/dfrobot_mmwave/），你需要跟我确定这其中要存什么数据？，如何绑定，例如确定了设备固件测唯一得mac地址，HA设备端唯一的设备ID,共同绑定生成一个设备号（这个设备号如何生成，删除设备后怎样处理，设备号不能太长））；这样处理后，前端就可以知道哪些设备已经是初始化绑定的，哪些是新设备未绑定，需要走绑定流程；

    2.2. 设备显示列表，设备ID太长了，改成显示设备号

    2.3. 扫描时，已经绑定的默认显示出来，未绑定的经过扫描后可显示出来，未绑定的不显示设备号（因为还没绑定嘛，没有设备号，可以显示个‘-’），因为没绑定，设备号没有生成

    2.4. 设备初始化绑定流程（已经开发的弹窗，还需完善功能）

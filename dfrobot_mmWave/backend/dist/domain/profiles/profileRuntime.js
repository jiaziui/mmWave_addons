...
exports.writeC4004Entity = exports.findWritableEntityId = exports.buildDeviceStateMap = exports.loadEntityRegistry = exports.resolveEntityId = exports.matchesDefinition = exports.getDefinition = exports.toEntityId = void 0;
...
const definitionSlugs = (definition) => [definition.slug, ...(definition.aliases ?? [])];
const escapeRegExp = (value) => value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
const matchesDefinition = (entityId, definition) => {
...
const resolveEntityId = (owner, definition, entityRegistryEntries = []) => {
    const canonicalEntityId = (0, exports.toEntityId)(owner.prefix, definition);
    ...
    const candidates = deviceEntries.filter((entry) => (0, exports.matchesDefinition)(entry.entity_id, definition));
    ...
    const prefixedCandidates = candidates.filter((entry) => {
        const objectId = objectIdFromEntityId(entry.entity_id);
        return expectedObjectIds.some((expectedObjectId) => objectId === expectedObjectId || new RegExp(`^${escapeRegExp(expectedObjectId)}_\\d+$`).test(objectId));
    });
...
const loadEntityRegistry = async (client) => {
    try {
...
exports.loadEntityRegistry = loadEntityRegistry;
const buildDeviceStateMap = (owner, statesById, entityRegistryEntries) => {
    ...
    for (const definition of entityDefinitions) {
        ...
        let actualEntityId;
        try {
            actualEntityId = (0, exports.resolveEntityId)(owner, definition, entityRegistryEntries);
        }
        catch {
            continue;
        }
...
exports.buildDeviceStateMap = buildDeviceStateMap;
...
const writeC4004Entity = async (client, owner, key, value, entityRegistryEntries) => {
    const definition = (0, exports.getDefinition)(key);
    if (!definition) {
        throw new Error(`Unknown entity key: ${key}`);
    }
    const registryEntries = entityRegistryEntries ?? await (0, exports.loadEntityRegistry)(client);
    const entityId = (0, exports.resolveEntityId)(owner, definition, registryEntries);

...
const DEFAULT_DEVICE_DATA_DIR = "/data/dfrobot_mmwave";
const DEFAULT_LEGACY_DATA_DIR = "/config/dfrobot_mmwave";
const DEFAULT_USER_BASE_MAP_DIR = "/config/dfrobot_mmwave/base_maps/user";
...
export interface AppConfig {
  port: number;
  /** @deprecated Use deviceDataDir. Kept for older frontend/debug consumers. */
  dataDir: string;
...
export const loadConfig = (): AppConfig => {
  ...
  const legacyDataDir = process.env.LEGACY_DATA_DIR?.trim() || DEFAULT_LEGACY_DATA_DIR;
  const deviceDataDir = process.env.DEVICE_DATA_DIR?.trim() || DEFAULT_DEVICE_DATA_DIR;
  const userBaseMapDir = process.env.USER_BASE_MAP_DIR?.trim() || DEFAULT_USER_BASE_MAP_DIR;

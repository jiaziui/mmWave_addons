    ...
    if (regionType === "status_detection" && event.tagType === "people_counting") {
      ...
      return {
        ...
        entry: {
          ...
          staticCount,
          totalCount,
          // English for HA/export readability; UI formats from structured fields via i18n.
          message: `${displayIndex} ${regionLabel}: moving ${movingCount}, static ${staticCount}, total ${totalCount}`,
        },
      };
    }
      ...
      return {
        ...
        entry: eventType
          ? {
              ...base,
              eventType,
              message: eventType === "approach"
                ? `${displayIndex} ${regionLabel}: someone approached`
                : `${displayIndex} ${regionLabel}: someone moved away`,
            }
          : null,
      };
    }
        ...
        entry: eventType
          ? {
              ...
              message: eventType === "enter"
                ? `${displayIndex} ${regionLabel}: someone entered`
                : `${displayIndex} ${regionLabel}: someone exited`,
            }
          : null,
      };
    }

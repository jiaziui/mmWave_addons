...
const readUInt16 = (bytes: number[], offset: number): number => (bytes[offset] << 8) | bytes[offset + 1];

const readSignBitInt16 = (bytes: number[], offset: number): number => {
  const raw = readUInt16(bytes, offset);
  return raw & 0x8000 ? -(raw & 0x7fff) : raw;
};

export interface LearnedRangePoint {
  x: number;
  y: number;
}

export const parseLearnedRangeHex = (value: string): LearnedRangePoint[] => {

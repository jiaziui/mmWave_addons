...
export interface MmwaveProfileAdapter {
  ...
  buildRuntimeState?(
    device: StoredMmwaveDevice,
    statesById: Map<string, HaEntityState>,
    options?: { forceSnapshot?: boolean; tagRegions?: Map<number, TagRegionRuntime> },
  ...
  /** After factory_reset button: re-apply install + panel parameter defaults to device. */
  applyFactoryDefaults?(client: HaClient, device: StoredMmwaveDevice): Promise<void>;
  ...
  clearLiveCount?(client: HaClient, device: StoredMmwaveDevice): Promise<void>;

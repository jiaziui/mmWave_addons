...
export interface ConfigFileRangePayload {
  pointCount: number;
  hex: string;
}
...
const writeUint16 = (value: number): string =>
  `${toHexByte((value >> 8) & 0xff)}${toHexByte(value & 0xff)}`;
...
export const buildConfigFileRangeHex = (regionConfig: StoredRegionConfig): ConfigFileRangePayload => {
...
export const assertRawCustomRangePointCount = (value: unknown): void => {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new Error("Invalid custom range: region config is required");
  }
  const detection = (value as Record<string, unknown>).detection;
  ...
  const points = (detection as Record<string, unknown>).customPointsCm;

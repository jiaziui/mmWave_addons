...
import { decodeMultipartFileName } from "../utils/multipartFileName";
  ...
  router.put("/base-maps/user/:assetId", (req, res) => {
    upload.single("file")(req, res, (uploadError) => {
      ...
      try {
        ...
        // Prefer UTF-8 text field from client; fall back to multer name with Latin-1 fix.
        const bodyName =
          typeof req.body?.originalName === "string" ? req.body.originalName.trim() : "";
        const originalName =
          bodyName || decodeMultipartFileName(req.file.originalname) || req.file.originalname;

...
const createRegionConfig = (variant: "kitchen" | "living" | "study"): StoredRegionConfig => {
  if (variant === "kitchen") {
    return {
      ...
      backgroundInstances: [],
      viewPreferences: { gridVisible: true, backgroundVisible: false, fillVisible: true },
  ...
  if (variant === "living") {
    return {
      ...
      viewPreferences: { gridVisible: true, backgroundVisible: true, fillVisible: true },
...
const baseDevices: StoredMmwaveDevice[] = [
  {
    ...
    deviceSettings: {
      trkLed: true,
      occLed: true,
      realTimeReportInterval: 2,
      trajectoryGenerationDistance: 50,
      trajectoryLifetime: 10,
      unoccupiedTime: 5,
    ...
    lastZoneSnapshot: {
      ...
      presenceStates: [],
      zones: [],
      counts: { liveCount: 2, targetCount: 2, movingCount: 1, staticCount: 1 },
    },
  },
  {
    id: "mock-c4004-2",
    ...
    lastZoneSnapshot: {
      ...
      presenceStates: [],
      zones: [],
      counts: { liveCount: 1, targetCount: 1, movingCount: 1, staticCount: 0 },
    },
  },
  {
    id: "mock-c4004-3",
    ...
    deviceSettings: {
      trkLed: false,
      occLed: true,
      realTimeReportInterval: 3,
      trajectoryGenerationDistance: 40,
      trajectoryLifetime: 8,
      unoccupiedTime: 20,
      frameGenerateCount: 5,
    },
    ...
    lastZoneSnapshot: {
      ...
      presenceStates: [],
      zones: [],
      counts: { liveCount: 0, targetCount: 0, movingCount: 0, staticCount: 0 },
    },
  },
  {
    id: "mock-c4004-4",
...
const overviewCard = (device: StoredMmwaveDevice, tick: number): MmwaveOverviewDeviceCard => {
  ...
  return {
    ...
    liveCount: movingCount + staticCount,
...
export const mockFetchMeta = async (): Promise<MetaConfig> => ({
  ...
  mqttConnected: true,
  dataDir: "memory-only",
  deviceDataDir: "memory-only/devices",
  userBaseMapDir: "memory-only/base-maps/user",
  legacyDataDir: "memory-only/legacy",
});
...
export const mockFetchOverview = async (): Promise<{ ok: boolean; metrics: MmwaveOverviewMetrics; devices: MmwaveOverviewDeviceCard[] }> => {
  ...
  return {
    ...
    metrics: {
      ...
      liveCount: cards.reduce((sum, card) => sum + card.liveCount, 0),
...
export const mockFetchDetail = async (deviceId: string): Promise<{ ok: boolean; detail: MmwaveDeviceDetail }> => {
  ...
  return {
    ...
    detail: {
      ...
      basics: (() => {
        const settings = devices[index].deviceSettings ?? {};
        const appliedMode = regionConfigs[deviceId].detection.appliedMode ?? regionConfigs[deviceId].detection.mode;
        return [
          { key: "installMode", label: "Install Mode", value: "Side" },
          {
            key: "installAngle",
            label: "Install Angle",
            value: `${devices[index].installInfo?.installAngleDeg ?? settings.installZAngle ?? 0}°`,
          },
          {
            key: "realTimeReportInterval",
            label: "Real-time Report Interval",
            value: `${settings.realTimeReportInterval ?? 2} s`,
          },
          {
            key: "installHeight",
            label: "Install Height",
            value: `${Math.round((devices[index].installInfo?.installHeightM ?? 1.8) * 100)} cm`,
          },
          {
            key: "trajectoryGenerationDistance",
            label: "Trajectory Generation Distance",
            value: `${settings.trajectoryGenerationDistance ?? 50} cm`,
          },
          {
          ...
          },
          {
            ...
            value: `${settings.trajectoryLifetime ?? 10} s`,
          },
          {
            key: "frameGenerateCount",
            label: "Frame Generation Count",
            value: `${settings.frameGenerateCount ?? 3}`,
          },
          {
            key: "unoccupiedTime",
            label: "Unoccupied Time",
            value: `${settings.unoccupiedTime ?? 5} s`,
          },
        ];
      })(),
...
const mockDeviceLogs = (deviceId: string): DeviceLogEntry[] => {
  ...
  return [
  {
    ...
    staticCount: 2,
    totalCount: 3,
    message: "1 办公区: moving 1, static 2, total 3",
  },
  {
    ...identity,
    ...
    regionType: "approach_depart",
    eventType: "approach",
    message: "2 卧室: someone approached",
  },
  {
    ...identity,
...
export const mockFactoryResetDevice = async (
  ...
  // 恢复出厂后上位机/设备统一默认四方：cm -200~200 / 0~400
  const current = regionConfigs[deviceId] ?? device.regionConfig;
  const factoryRangeBox = { xMin: -2, xMax: 2, yMin: 0, yMax: 4 };
  ...
  device.regionConfig = regionConfigs[deviceId];
  device.installInfo = { installMode: "side", installAngleDeg: 0, installHeightM: 1.8 };
...
export const mockClearLiveCount = async (
  deviceId: string,
...
export const mockInitializeDevice = async (
  ...
  device.deviceSettings = {
    trkLed: true,
    occLed: payload.detectionMode === 1,
    realTimeReportInterval: 2,
    trajectoryGenerationDistance: 50,
    trajectoryLifetime: 10,
    unoccupiedTime: payload.detectionMode === 1 ? 5 : 30,
    frameGenerateCount: payload.detectionMode === 1 ? 2 : 7,
  };

...
export interface C4004DeviceSettings {
  ...
  realTimeReportInterval?: number;
  trajectoryGenerationDistance?: number;
  trajectoryLifetime?: number;
  frameGenerateCount?: number;
  unoccupiedTime?: number;
  zone1McuIo?: number;
  zone2McuIo?: number;
  zone3McuIo?: number;
  zone4McuIo?: number;
...
export interface StoredZoneSnapshot {
  ...
  counts: { liveCount: number; targetCount: number; movingCount: number; staticCount: number };
}
...
export const clearLiveCount = async (deviceId: string): Promise<{ ok: boolean; detail: MmwaveDeviceDetail }> =>
  localMockEnabled() ? (await mockApi()).mockClearLiveCount(deviceId) : postDeviceAction(deviceId, "clear-live-count");
...
export const uploadUserBaseMap = async (assetId: string, file: File): Promise<{ ok: boolean; asset: UserBaseMapAsset }> => {
  ...
  // Explicit UTF-8 field — multipart Content-Disposition filenames are often Latin-1 mangled

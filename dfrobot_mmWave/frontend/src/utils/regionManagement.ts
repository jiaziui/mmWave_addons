...
export type RegionTranslate = (key: string, params?: Record<string, string | number>) => string;

export const formatRegionLiveInfo = (
  regionType: RegionType,
  live?: Pick<RegionOverlay, "movingCount" | "staticCount" | "boundaryState" | "approachAwayState" | "tagDataAvailable" | "tagTypeMismatch">,
  t?: RegionTranslate,
) => {
  const tr = t ?? ((key, params) => {
    const fallback: Record<string, string> = {
      ...
      "region.live.movingStatic": `运动 ${params?.moving ?? 0}  静止 ${params?.static ?? 0}`,
      "region.live.away": "远离",
      "region.live.none": "无",
    ...
    if (key === "region.live.movingStatic") {
      return `运动 ${params?.moving ?? 0}  静止 ${params?.static ?? 0}`;
    }
    return fallback[key] ?? key;
  });
  if (regionType === "noise" || regionType === "empty_tag") return "";
  if (!live) return "";
  if (live.tagTypeMismatch) return tr("region.live.tagTypeMismatch");
  if (live.tagDataAvailable === false) return tr("region.live.waitTag");
  if (regionType === "status_detection") {
    return tr("region.live.movingStatic", { moving: live.movingCount ?? 0, static: live.staticCount ?? 0 });
  }
  if (regionType === "approach_depart") {
    const state = (live.approachAwayState ?? "").toLowerCase();
    if (state.includes("away") || state.includes("远离") || state === "depart") return tr("region.live.away");
    if (state.includes("none") || state.includes("无")) return tr("region.live.none");
    return state.includes("approach") || state.includes("靠近") ? tr("region.live.approach") : live.approachAwayState ?? "";
  }
  if (regionType === "boundary") {
    const state = (live.boundaryState ?? "").toLowerCase();
    if (state.includes("out") || state.includes("出")) return tr("region.live.out");
    if (state.includes("none") || state === "2") return tr("region.live.none");
    return state.includes("in") || state.includes("进") ? tr("region.live.in") : live.boundaryState ?? "";
  }
  return "";
};

export const getDetectionHint = (detection: DetectionRangeConfig, t?: RegionTranslate) => {
  const tr = t ?? ((key, params) => {
    if (key === "region.detection.hint.rect") return "在右侧坐标轴拖拽矩形边角调整四方范围，完成后点击设置。";
    if (key === "region.detection.hint.customConfirmed") return `已确认 ${params?.count ?? 0} 点，点击设置启用。`;
    if (key === "region.detection.hint.customReady") return `已选 ${params?.count ?? 0} 点，可继续加点或点击设置。`;
    if (key === "region.detection.hint.customPartial") return `已选 ${params?.count ?? 0} 点，请在右侧继续点击。`;
    if (key === "region.detection.hint.customEmpty") return "在右侧坐标轴点击设置自定义范围起点。";
    return key;
  });
  if (detection.mode === "rect") {
    return tr("region.detection.hint.rect");
  }
  if (detection.mode === "learned") {
    return "";
  }
  if (detection.customConfirmed) {
    return tr("region.detection.hint.customConfirmed", { count: detection.customPointsCm.length });
  }
  if (detection.customPointsCm.length >= 3) {
    return tr("region.detection.hint.customReady", { count: detection.customPointsCm.length });
  }
  if (detection.customPointsCm.length > 0) {
    return tr("region.detection.hint.customPartial", { count: detection.customPointsCm.length });
  }
  return tr("region.detection.hint.customEmpty");
};
...
export const parseRegionIni = (content: string): RegionDefinition[] => {
  ...
  lines.forEach((rawLine, offset) => {
    ...
    if (regionType !== "status_detection" && ioIndex !== 0) {
      throw new Error(`第 ${lineNumber} 行只有人数统计区域可以绑定IO索引`);
    }

...
import {
  getSystemBaseMapDisplayName,
  listSystemBaseMapAssets,
  resolveBaseMapSourceUrl,
} from "../utils/baseMapAssets";
...
const REGION_LABEL_KEYS: Record<RegionType, string> = {
  status_detection: "region.type.status_detection",
  noise: "region.type.noise",
  approach_depart: "region.type.approach_depart",
  boundary: "region.type.boundary",
  empty_tag: "region.type.empty_tag",
};
...
const parameterFields = [
  { key: "realTimeReportInterval", labelKey: "region.params.realTimeReportInterval", min: 1, max: 3600 },
  { key: "trajectoryGenerationDistance", labelKey: "region.params.trajectoryGenerationDistance", min: 0, max: 1000 },
  { key: "trajectoryLifetime", labelKey: "region.params.trajectoryLifetime", min: 0, max: 600 },
  { key: "unoccupiedTime", labelKey: "region.params.unoccupiedTime", min: 5, max: 3600 },
  { key: "frameGenerateCount", labelKey: "region.params.frameGenerateCount", min: 1, max: 64 },
] as const;
...
const resolveViewPreferences = (config: StoredRegionConfig) => ({
  ...
  fillVisible: config.viewPreferences?.fillVisible ?? true,
});
...
export function RegionManagementPage({
  ...
  const [backgroundMessage, setBackgroundMessage] = useState(() => t("region.bg.hint.default"));
  ...
  useEffect(() => {
    ...
    Promise.allSettled([fetchDeviceConfig(selectedDevice.id), fetchDeviceDetail(selectedDevice.id), fetchUserBaseMaps()])
      .then(([configResult, detailResult, mapsResult]) => {
        ...
        setFillVisible(prefs.fillVisible);
      ...
      .catch((error: unknown) => {
        if (!cancelled) onError(error instanceof Error ? error.message : t("region.err.loadFailed"));
      })
      .finally(() => {
  ...
  useEffect(() => {
    setBackgroundMessage(t("region.bg.hint.default"));
  }, [locale, t, selectedDevice?.id]);
  ...
  const operateLearnedRange = async (action: "start" | "stop" | "query") => {
    ...
    try {
      ...
      onError(error instanceof Error ? error.message : t("region.err.learnFailed"));
    } finally {
      if (activeDeviceIdRef.current === targetDeviceId) {
        setLearningActionBusy(false);
      }
  ...
  const sourceUrl = (sourceType: "system" | "user", sourceId: string) =>
    resolveBaseMapSourceUrl(sourceType, sourceId);

  const isSupportedBackgroundImage = (file: File): boolean => {
  ...
  const uploadBackgroundFile = async (file: File) => {
    ...
    if (!isSupportedBackgroundImage(file)) {
      const tip = t("region.err.unsupportedImageType");
      setBackgroundMessage(t("region.bg.hint.unsupportedType"));
      onError(tip);
      return;
    }
    ...
    try {
      ...
      setBackgroundMessage(t("region.bg.hint.imported"));
      onMessage(t("region.msg.imageSaved"));
    } catch (error) {
      const message = error instanceof Error ? error.message : t("region.err.uploadFailed");
      const looksUnsupported =
        /unsupported|invalid image|mime|format|不支持|格式/i.test(message);
      const tip = looksUnsupported ? t("region.err.unsupportedImageType") : message;
      setBackgroundMessage(looksUnsupported ? t("region.bg.hint.unsupportedType") : tip);
      onError(tip);
    } finally {
      setSaving(false);
    }
  ...
  const removeImportedBackgroundSource = async (sourceId: string) => {
    ...
    if (!source) return;
    const confirmed = window.confirm(t("region.bg.confirmDelete", { name: source.name }));
    if (!confirmed) return;
    ...
    if (deviceConfig?.regionConfig) {
      ...
      if (next.backgroundInstances.length !== before) {
        ...
        if (!saved) {
          onError(t("region.err.canvasSyncAfterDelete"));
        }
      }
    }

    setSelectedBackgroundId(null);
    setBackgroundMessage(t("region.bg.hint.deleted"));
    onMessage(t("region.msg.imageDeleted"));
  };
  ...
  const removeUserBaseMapAsset = async (assetId: string) => {
    ...
    if (!asset) return;
    const confirmed = window.confirm(t("region.bg.confirmDelete", { name: asset.originalName }));
    if (!confirmed) return;
    ...
    try {
      ...
      setSaving(false);
      onError(error instanceof Error ? error.message : t("region.err.deleteFailed"));
      return;
    }
    ...
    if (deviceConfig?.regionConfig) {
      ...
      if (next.backgroundInstances.length !== before) {
        const saved = await persistRegionConfig(next, undefined, { quiet: true });
        if (!saved) {
          onError(t("region.err.canvasSyncAfterAssetDelete"));
        }
      }
    }
  ...
  const persistRegionConfig = async (
    ...
    try {
      ...
      if (apply?.customRange && response.applyResult.customRange !== "applied") {
        setCurrentConfig(optimistic);
        setDirty(true);
        onError(response.applyResult.warnings.join("；") || t("region.err.customSyncFailed"));
        return false;
      }
      if (apply?.fourSidedRange && response.applyResult.fourSidedRange !== "applied") {
        ...
        setDirty(true);
        onError(response.applyResult.warnings.join("；") || t("region.err.rectSyncFailed"));
        return false;
      }
      ...
      if (failedRequiredApply) {
        ...
        onError(
          response.applyResult.warnings.join("；") ||
            failedRequiredApply === "regionMcuIo" ? t("region.err.applyRegionMcuIo") : t("region.err.applyTagConfig"),
        );
        return false;
      }
      ...
      if (!options?.quiet) {
        ...
        if (response.applyResult.warnings.length) {
          ...
          onMessage(t("region.msg.configSaved"));
        }
      }
      return true;
    } catch (error) {
      ...
      onError(error instanceof Error ? error.message : t("region.err.saveFailed"));
      return false;
    } finally {
      if (activeDeviceIdRef.current === targetDeviceId) {
        setSaving(false);
  ...
  const applyViewPreferences = (patch: Partial<{ gridVisible: boolean; backgroundVisible: boolean; fillVisible: boolean }>) => {
    ...
    if (patch.fillVisible !== undefined) setFillVisible(patch.fillVisible);
  ...
  const updateSettings = async (updates: C4004DeviceSettings) => {
    ...
    try {
      ...
      if (activeDeviceIdRef.current !== targetDeviceId) return;
      setCurrentConfig(response.config);
      onMessage(t("region.msg.settingsSynced"));
    } catch (error) {
      onError(error instanceof Error ? error.message : t("region.err.settingsSyncFailed"));
    } finally {
      if (activeDeviceIdRef.current === targetDeviceId) {
        setSaving(false);
      }
  ...
  const handleFactoryReset = async () => {
    ...
    try {
      ...
      if (activeDeviceIdRef.current !== targetDeviceId) return;
      // 以上位机返回的配置为准（含强制四方后的 rangeBox / appliedMode）
      setCurrentConfig({
        ...response.config,
        deviceSettings: {
          ...PARAM_DEFAULTS,
          ...response.config.deviceSettings,
        },
      });
      setDetail((current) =>
        current
      ...
      detectionBaselineRef.current = null;
      onMessage(t("region.msg.factoryResetDone"));
      // 再拉一次 config + 总览/详情，确保画布、探测范围模式与 basics 都刷新
      const [configResult] = await Promise.allSettled([
        fetchDeviceConfig(targetDeviceId),
        refreshDetail(),
        onDevicesChanged?.(),
      ]);
      if (activeDeviceIdRef.current === targetDeviceId && configResult.status === "fulfilled") {
        const config = configResult.value.config;
        setCurrentConfig({
          ...config,
          deviceSettings: {
        ...
        setDetail((current) =>
          current
            ? {
                ...current,
                regions: [],
                detection: config.regionConfig.detection,
                rangeBox: config.regionConfig.rangeBox,
              }
            : current,
        );
      }
    } catch (error) {
      onError(error instanceof Error ? error.message : t("region.err.factoryResetFailed"));
    } finally {
      if (activeDeviceIdRef.current === targetDeviceId) {
        setSaving(false);
      }
  ...
  const saveOverallRegion = async () => {
    ...
    try {
      ...
      onMessage(t("region.msg.overallMcuIoSynced"));
    } catch (error) {
      onError(error instanceof Error ? error.message : t("region.err.overallMcuIoFailed"));
    } finally {
  ...
  const addRegion = () => {
    ...
    const region: RegionDefinition = {
      id: createClientId(),
      index,
      label: t("region.defaultLabel", { n: index + 1 }),
    ...
    try {
      ...
      if (importType === "detection") {
        ...
        if (!canExportCustomRange(points)) {
          throw new Error(t("region.err.customPointsInvalid"));
        }
        next.detection = {
          ...next.detection,
          mode: "custom",
        ...
        onMessage(t("region.msg.customImported"));
        return;
      }
      const normalizedText = text.replace(/^\uFEFF/, "").trimStart();
      ...
      clearRegionEditSession();
      setActivePanel("regions");
      onMessage(t("region.msg.regionsImported"));
    } catch (error) {
      onError(error instanceof Error ? error.message : t("region.err.importFailed"));
    }
  };
  ...
  const handleMenuAction = (
    ...
    if (action === "clear-live-count") {
      if (readOnly) {
        onError(t("region.err.offlineClearLiveCount"));
        return;
      }
      void (async () => {
        const targetDeviceId = selectedDevice.id;
        setSaving(true);
        try {
          const response = await clearLiveCount(targetDeviceId);
          if (activeDeviceIdRef.current !== targetDeviceId) return;
          setDetail(response.detail);
          onMessage(t("region.msg.liveCountCleared"));
        } catch (error) {
          onError(error instanceof Error ? error.message : t("region.err.clearLiveCountFailed"));
        } finally {
          if (activeDeviceIdRef.current === targetDeviceId) {
            setSaving(false);
          }
    ...
    if ((action === "export-detection" || action === "export-regions") && readOnly) {
      onError(t("region.err.offlineExport"));
      return;
    }
  ...
  const backgroundCatalog: BackgroundCatalogItem[] = [
    ...
    ...libraryAssets.map((asset) => ({
      key: `asset:${asset.id}`,
      name: asset.sourceType === "system"
        ? getSystemBaseMapDisplayName(asset.id, locale)
        : asset.name,
      url: asset.url,
      kind: "asset" as const,
      id: asset.id,
  ...
  const backgroundFilterOptions: Array<{ id: BackgroundCatalogFilter; label: string; count: number }> = [
    { id: "all", label: t("region.bg.filter.all"), count: backgroundCatalog.length },
    { id: "system", label: t("region.bg.filter.system"), count: systemAssets.length },
    { id: "user", label: t("region.bg.filter.user"), count: importedBackgroundSources.length + userAssets.length },
  ];
  ...
  const selectBackgroundSourceByKey = (key: string) => {
    ...
    setBackgroundMessage(t("region.bg.hint.selected"));
  };
  ...
  const placeBackgroundItem = async (item: BackgroundCatalogItem) => {
    ...
    if (saved) {
      ...
      setBackgroundMessage(t("region.bg.hint.added"));
      onMessage(t("region.msg.backgroundAdded"));
    }
  };

  const addSelectedBackground = async () => {
    if (!currentCatalogItem) {
      setBackgroundMessage(t("region.bg.hint.none"));
      return;
    }
  ...
  useEffect(() => {
    const handleCanvasDelete = (event: KeyboardEvent) => {
      ...
      if (activePanel === "background" && selectedBackgroundId) {
      ...
      if (
        !selectedRegionId
        || selectedRegionId === OVERALL_REGION_ID
        || !regionConfig?.regions.some((entry) => entry.id === selectedRegionId)
      ) return;
      event.preventDefault();
      void deleteRegion(selectedRegionId);
    };
    window.addEventListener("keydown", handleCanvasDelete);
    return () => window.removeEventListener("keydown", handleCanvasDelete);
  }, [
    activePanel,
    readOnly,
    saving,
  ...
  const processPointerMove = (event: PointerMoveSample) => {
    ...
    if (activeDrag.kind === "background-pending") {
      ...
      const moved = promoteBackgroundPending(activeDrag);
      activeDrag = moved;
      const current = configRef.current;
      if (!current) return;
      const next = cloneConfig(current.regionConfig);
      const instance = next.backgroundInstances.find((entry) => entry.id === moved.instanceId);
      if (!instance) return;
      instance.xCm = Math.round(moved.instance.xCm + world.x - moved.startWorldX);
      instance.yCm = Math.round(moved.instance.yCm + world.y - moved.startWorldY);
      setCurrentConfig({ ...current, regionConfig: next });
      setDirty(true);
      return;
    }
  ...
  const renderRegionShape = (region: RegionDefinition) => {
    ...
    const label = formatRegionLiveInfo(displayRegion.regionType, live, t);
          ...
          {onBackToOverview ? (
            <div className="page-actions">
              <button type="button" className="ghost-button region-back-overview-btn" onClick={onBackToOverview}>
                {t("region.backToOverview")}
              </button>
            </div>
          ) : null}
        </header>
        <div className="empty-state"><strong>{t("region.empty.noDevices.title")}</strong><span>{t("region.empty.noDevices.hint")}</span></div>
      </section>
    );
  }
  if (!regionConfig || loading) {
    return (
      <section className="page region-page">
        ...
        <div className="empty-state"><strong>{t("region.empty.loading.title")}</strong><span>{selectedDevice.name}</span></div>
      </section>
    );
  }
  ...
  const workspaceClass = [
    ...
    fillVisible ? "" : "is-fill-hidden",
  ].filter(Boolean).join(" ");
  // 探测面板打开时按当前配置 Tab 预览对应范围（四方/轨迹/自定义互斥显示）；
  // 关闭面板后仍显示固件已生效的 appliedMode。
  ...
  const learnedDisplayPoints = (() => {
    if (visibleDetectionMode !== "learned") {
      return [] as Array<{ x: number; y: number }>;
    }
    // 固件当前生效为四方/自定义时，切到轨迹 Tab 不预览历史学习范围；
    // 仅学习进行中显示 runtime 实时点。
    if (appliedDetectionMode !== "learned") {
      if (learningLocked && (learnedRange?.pointsCm.length ?? 0) >= 3) {
        return learnedRange!.pointsCm;
      }
      return [];
    }
    if ((learnedRange?.pointsCm.length ?? 0) >= 3) {
      return learnedRange!.pointsCm;
    }
    return detection.learnedPointsCm;
  })();
    ...
    <section className="page region-page">
      <header className="page-header region-page-header">
        <div className="region-page-title-row">
          ...
          <h2>{t("region.title")}</h2>
          <div className="region-device-picker">
            <span className="region-device-name">{selectedDevice.name}{readOnly ? t("region.offline.suffix") : ""}</span>
            <div className="region-deviceno-wrap" ref={devicePickerWrapRef}>
              <button type="button" className="region-deviceno-btn" aria-expanded={devicePickerOpen} onClick={() => setDevicePickerOpen((value) => !value)}>
                <span className="region-deviceno-btn-label">{t("region.deviceNo")} <strong>{selectedDevice.deviceNo}</strong></span>
              ...
              {devicePickerOpen ? <div className="region-deviceno-dropdown" role="listbox">
                {boundDevices.map((device) => <button type="button" className={device.id === selectedDevice.id ? "active" : ""} key={device.id} onClick={() => {
                  if (dirty && !window.confirm(t("region.confirm.switchDevice"))) return;
                  setDevicePickerOpen(false);
                  onSelectDevice(device.id);
                }}><span>{t("region.deviceNo")} {device.deviceNo}</span><small>{device.name}{device.discovery.status === "offline" ? ` ${t("region.offline.dot")}` : ""}</small></button>)}
              </div> : null}
            </div>
          </div>
        </div>
        <div className="page-actions">
          {saving ? <span className="save-state">{t("region.saveState.saving")}</span> : dirty ? <span className="save-state pending">{t("region.saveState.dirty")}</span> : null}
          {onBackToOverview ? (
            <button
              type="button"
              className="ghost-button region-back-overview-btn"
              onClick={() => {
                if (dirty && !window.confirm(t("region.confirm.leave"))) return;
                onBackToOverview();
              }}
            >
              {t("region.backToOverview")}
          ...
          <div className="region-menu-wrap" ref={menuWrapRef}>
            <button type="button" className="region-menu-btn" aria-label={t("region.menu.more")} onClick={() => setMenuOpen((value) => !value)}>⋯</button>
            {menuOpen ? <div className="region-dropdown">
              <button type="button" disabled={readOnly} onClick={() => handleMenuAction("import-image")}>{t("region.menu.importImage")}</button>
              <button type="button" disabled={readOnly} onClick={() => handleMenuAction("import-detection")}>{t("region.menu.importDetection")}</button>
              <button type="button" onClick={() => handleMenuAction("export-detection")}>{t("region.menu.exportDetection")}</button>
              <button type="button" disabled={readOnly} onClick={() => handleMenuAction("import-regions")}>{t("region.menu.importRegions")}</button>
              <button type="button" onClick={() => handleMenuAction("export-regions")}>{t("region.menu.exportRegions")}</button>
              <button type="button" disabled={readOnly || saving} onClick={() => handleMenuAction("clear-live-count")}>{t("region.menu.clearLiveCount")}</button>
            </div> : null}
            <input ref={fileInputRef} className="region-bg-file-input" type="file" accept="image/png,image/jpeg,image/webp,.png,.jpg,.jpeg,.webp" multiple onChange={(event) => {
      ...
      <div className={workspaceClass}>
        ...
        <div className="region-toolbar">
          <div className="region-toolbar-group">
            <button className={gridVisible ? "region-tool-btn active" : "region-tool-btn"} type="button" onClick={() => applyViewPreferences({ gridVisible: !gridVisible })}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 8h16M4 16h16M8 4v16M16 4v16" /></svg><span>{t("region.tool.grid")}</span></button>
            <button className={backgroundVisible ? "region-tool-btn active" : "region-tool-btn"} type="button" onClick={() => applyViewPreferences({ backgroundVisible: !backgroundVisible })}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="4" y="5" width="16" height="14" rx="2" /><circle cx="9" cy="10" r="1.5" fill="currentColor" stroke="none" /><path d="m5 17 4-4 3 3 4-5 3 4" /></svg><span>{t("region.tool.background")}</span></button>
            <button className={fillVisible ? "region-tool-btn active" : "region-tool-btn"} type="button" onClick={() => applyViewPreferences({ fillVisible: !fillVisible })}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M19 11H5l7-7 7 7Z" /><path d="M5 11v8a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-8" /><path d="M9 21v-4h6v4" /></svg><span>{t("region.tool.fill")}</span></button>
            <button className="region-tool-btn" type="button" onClick={() => { setCellSize(BASE_CELL); setViewportOffset({ x: canvasSize.width / 2, y: canvasSize.height / 2 }); }}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 12a9 9 0 1 0 3-6.7" /><path d="M3 4v5h5M12 8v4l3 2" /></svg><span>{t("region.tool.reset")}</span></button>
          </div>
          <div className="region-toolbar-group">
            <button className={activePanel === "regions" ? "region-panel-btn active" : "region-panel-btn"} type="button" onClick={() => togglePanel("regions")}>{t("region.panelBtn.regions")}</button>
            <button className={activePanel === "detection" ? "region-panel-btn active" : "region-panel-btn"} type="button" onClick={() => togglePanel("detection")}>{t("region.panelBtn.detection")}</button>
          </div>
          <div className="region-toolbar-group">
            <button className={activePanel === "parameters" ? "region-panel-btn active" : "region-panel-btn"} type="button" onClick={() => togglePanel("parameters")}>{t("region.panelBtn.parameters")}</button>
            <button className={activePanel === "background" ? "region-panel-btn active" : "region-panel-btn"} type="button" onClick={() => togglePanel("background")}>{t("region.panelBtn.background")}</button>
          </div>
        </div>
        ...
        {activePanel ? <aside className={`region-float-panel region-${activePanel}-panel`}>
          {activePanel === "regions" ? <>
            <div className="region-panel-head">
              <h3>{t("region.panel.regions")}</h3>
              <div className="region-panel-head-actions">
                <button type="button" className="region-sync-btn" onClick={() => void syncAllRegions()} disabled={readOnly || saving}>{t("region.action.sync")}</button>
                <button type="button" className="region-panel-close" aria-label={t("common.close")} onClick={() => togglePanel("regions")}>×</button>
              </div>
            </div>
            <div className="region-list-body">
              <div className={selectedRegionId === OVERALL_REGION_ID ? "region-list-item active" : "region-list-item"} onClick={selectOverallRegion}>
                ...
                <div className="region-list-meta">
                  <strong>{t("region.list.overall")}</strong>
                </div>
                <div className="region-list-actions">
                  <button type="button" className="region-icon-btn" disabled={readOnly} aria-label={t("region.aria.editOverallMcuIo")} onClick={(event) => { event.stopPropagation(); selectOverallRegion(); }}>✎</button>
                </div>
              </div>
                  ...
                  return (
                    <div className={selectedRegionId === region.id ? "region-list-item active" : "region-list-item"} key={region.id} onClick={() => selectRegion(region)}>
                      ...
                      <div className="region-list-meta">
                        ...
                        <span>
                          <em>{t("region.list.index", { index: displayRegion.index })}</em>
                          <em>{t(REGION_LABEL_KEYS[displayRegion.regionType])}</em>
                        </span>
                      </div>
                      <div className="region-list-actions">
                        <button type="button" className="region-icon-btn" disabled={readOnly} aria-label={t("region.aria.toggleVisibility")} onClick={(event) => { event.stopPropagation(); void toggleRegionVisibility(region.id); }}>{region.visible ? "👁" : "🚫"}</button>
                        <button type="button" className="region-icon-btn" disabled={readOnly} aria-label={t("region.aria.edit")} onClick={(event) => { event.stopPropagation(); selectRegion(region); }}>✎</button>
                        <button type="button" className="region-icon-btn danger" disabled={readOnly} aria-label={t("region.aria.delete")} onClick={(event) => { event.stopPropagation(); void deleteRegion(region.id); }}>🗑</button>
                      </div>
                    </div>
                  );
                }) : <div className="region-panel-empty">{t("region.list.empty")}</div>}
            </div>
            <div className="region-panel-foot">
              <span>{t("region.list.total", { count: regionConfig.regions.length + 1 })}</span>
              <button type="button" className="region-add-btn" onClick={addRegion} disabled={readOnly || regionConfig.regions.length >= 32}>{t("region.action.add")}</button>
            </div>
          </> : null}

          {activePanel === "edit" && overallMcuIoDraft !== null ? <>
            <div className="region-panel-head">
              <h3>{t("region.form.overallTitle")}</h3>
              <button type="button" className="region-panel-close" aria-label={t("common.close")} onClick={() => { clearRegionEditSession(); setActivePanel("regions"); }}>×</button>
            </div>
            <div className="region-form-grid">
              <label className="region-form-full"><span>{t("region.form.overallMcuIo")}</span><select disabled={readOnly || saving} value={overallMcuIoDraft} onChange={(event) => setOverallMcuIoDraft(Number(event.target.value))}>{MCU_IO_OPTIONS.map((value) => <option value={value} key={value}>{value < 0 ? t("region.form.none") : value}</option>)}</select></label>
            </div>
            <div className="region-panel-actions"><button type="button" className="primary-button" disabled={readOnly || saving} onClick={() => void saveOverallRegion()}>{t("region.action.saveAndSync")}</button><button type="button" className="ghost-button" onClick={() => { clearRegionEditSession(); setActivePanel("regions"); }}>{t("region.action.cancel")}</button></div>
          </> : null}

          {activePanel === "edit" && regionDraft ? <>
            <div className="region-panel-head">
              <h3>{t("region.form.regionTitle")}</h3>
              <button type="button" className="region-panel-close" aria-label={t("common.close")} onClick={() => discardRegionEdits()}>×</button>
            </div>
            <div className="region-form-grid">
              <label><span>{t("region.form.label")}</span><input disabled={readOnly} value={regionDraft.label} onChange={(event) => updateRegionDraft({ ...regionDraft, label: event.target.value })} /></label>
              <label><span>{t("region.form.index")}</span><input disabled={readOnly} type="number" min="0" max="31" value={regionDraft.index} onChange={(event) => {
                const index = Math.max(0, Math.min(31, Number(event.target.value)));
                updateRegionDraft({ ...regionDraft, index });
              }} /></label>
              <label><span>{t("region.form.type")}</span><select disabled={readOnly} value={regionDraft.regionType} onChange={(event) => {
              ...
              }}>{(Object.keys(REGION_LABEL_KEYS) as RegionType[]).map((value) => <option value={value} key={value}>{t(REGION_LABEL_KEYS[value])}</option>)}</select></label>
              <label><span>{t("region.form.shape")}</span><select disabled={readOnly} value={regionDraft.geometry.shape} onChange={(event) => {
              ...
              }}><option value="rect">{t("region.form.shape.rect")}</option><option value="circle">{t("region.form.shape.circle")}</option></select></label>
              <label className={regionDraft.regionType === "status_detection" ? undefined : "disabled-field"}><span>{t("region.form.ioIndex")}</span><select disabled={readOnly || regionDraft.regionType !== "status_detection"} value={regionDraft.ioIndex} onChange={(event) => {
              ...
              }}>{[0, 2, 3, 4, 5, 6].map((value) => <option value={value} key={value} disabled={value !== 0 && usedStatusIoIndexes.has(value as RegionDefinition["ioIndex"])}>{value === 0 ? t("region.form.none") : `IO${value}`}</option>)}</select></label>
              <label className={regionDraft.regionType === "status_detection" && regionDraft.ioIndex !== 0 ? undefined : "disabled-field"}><span>{t("region.form.mcuIo")}</span><select disabled={readOnly || regionDraft.regionType !== "status_detection" || regionDraft.ioIndex === 0} value={regionDraft.mcuIo} onChange={(event) => updateRegionDraft({ ...regionDraft, mcuIo: Number(event.target.value) })}>{MCU_IO_OPTIONS.map((value) => <option value={value} key={value}>{value < 0 ? t("region.form.none") : value}</option>)}</select></label>
              <label><span>{t("region.form.centerX")}</span><input disabled={readOnly} type="number" value={regionDraft.geometry.centerXCm} onChange={(event) => updateRegionDraft({ ...regionDraft, geometry: updateGeometryCenter(regionDraft.geometry, Number(event.target.value), regionDraft.geometry.centerYCm) })} /></label>
              <label><span>{t("region.form.centerY")}</span><input disabled={readOnly} type="number" value={regionDraft.geometry.centerYCm} onChange={(event) => updateRegionDraft({ ...regionDraft, geometry: updateGeometryCenter(regionDraft.geometry, regionDraft.geometry.centerXCm, Number(event.target.value)) })} /></label>
              {regionDraft.geometry.shape === "rect" ? <>
                <label><span>{t("region.form.width")}</span><input disabled={readOnly} type="number" min="10" value={regionDraft.geometry.widthCm} onChange={(event) => updateRegionDraft({ ...regionDraft, geometry: { ...regionDraft.geometry, widthCm: Math.max(10, Number(event.target.value)) } as RegionGeometry })} /></label>
                <label><span>{t("region.form.height")}</span><input disabled={readOnly} type="number" min="10" value={regionDraft.geometry.heightCm} onChange={(event) => updateRegionDraft({ ...regionDraft, geometry: { ...regionDraft.geometry, heightCm: Math.max(10, Number(event.target.value)) } as RegionGeometry })} /></label>
              </> : <label><span>{t("region.form.radius")}</span><input disabled={readOnly} type="number" min="10" value={regionDraft.geometry.radiusCm} onChange={(event) => updateRegionDraft({ ...regionDraft, geometry: { ...regionDraft.geometry, radiusCm: Math.max(10, Number(event.target.value)) } as RegionGeometry })} /></label>}
            </div>
            <div className="region-panel-actions"><button type="button" className="primary-button" disabled={readOnly || saving} onClick={() => void saveRegionDraft()}>{t("region.action.saveChanges")}</button><button type="button" className="ghost-button" onClick={() => discardRegionEdits()}>{t("region.action.cancel")}</button></div>
          </> : null}

          {activePanel === "parameters" ? <>
            <div className="region-panel-head">
              <div>
                <h3>{t("region.panel.parameters")}</h3>
                {readOnly ? <span>{t("region.params.readonly")}</span> : null}
              </div>
              <div className="region-panel-head-actions">
                <button type="button" className="region-panel-close" aria-label={t("common.close")} onClick={() => togglePanel("parameters")}>×</button>
              </div>
            </div>
            <div className="region-panel-scroll">
              <section className="parameter-section">
                <h4>{t("region.params.ledSection")}</h4>
                {([
                  ["trkLed", "region.params.trkLed"],
                  ["occLed", "region.params.occLed"],
                ] as const).map(([key, titleKey]) => (
                  <div className="region-range-switch-row" key={key}>
                    <div><strong>{t(titleKey)}</strong></div>
                    <button
                      type="button"
              ...
              <section className="parameter-section">
                <h4>{t("region.params.reportSection")}</h4>
                <div className="region-form-grid region-params-form-grid">
                  {parameterFields.map((field) => (
                    <label key={`${selectedDevice.id}-${field.key}-${deviceConfig?.deviceSettings[field.key] ?? PARAM_DEFAULTS[field.key]}`}>
                      ...
                      <input
                        ...
                        min={field.min}
                        max={field.max}
                        defaultValue={deviceConfig?.deviceSettings[field.key] ?? PARAM_DEFAULTS[field.key]}
                        onBlur={(event) => void updateSettings({ [field.key]: Math.max(field.min, Math.min(field.max, Number(event.target.value))) })}
              ...
              <section className="parameter-section region-factory-reset-section">
                <button
                  type="button"
                  className="ghost-button region-reset-params-btn"
                  disabled={readOnly || saving}
                  onClick={() => void handleFactoryReset()}
                >
                  {t("region.action.factoryReset")}
                </button>
              </section>
            </div>
            <div className="region-param-footer">
              <button type="button" className="primary-button region-reset-params-btn" disabled={readOnly || saving} onClick={() => void syncSettings()}>{t("region.action.sync")}</button>
            </div>
          </> : null}

          {activePanel === "background" ? <>
            <div className="region-panel-head">
              <div><h3>{t("region.panel.background")}</h3></div>
              <div className="region-panel-head-actions">
                <button type="button" disabled={readOnly} onClick={() => fileInputRef.current?.click()}>{t("region.action.upload")}</button>
                <button type="button" className="region-panel-close" aria-label={t("common.close")} onClick={() => togglePanel("background")}>×</button>
              </div>
            </div>
            <section className="region-param-section background-library-section">
              <div className="region-param-section-title">
                <h4>{t("region.bg.library")}</h4>
              </div>
              {backgroundCatalog.length ? (
                <>
                  <div className="bg-library-tabs" role="tablist" aria-label={t("region.bg.filter.aria")}>
                  ...
                  <div className="bg-material-grid" ref={baseMapLibraryRef}>
                    {visibleBackgroundCatalog.length ? visibleBackgroundCatalog.map((item) => {
                        ...
                        >
                          ...
                          {isImported || isUploadedAsset ? (
                            <button
                              ...
                              title={t("region.bg.deleteTitle")}
                              aria-label={t("region.bg.deleteAria", { name: item.name })}
                              onClick={(event) => {
                                event.preventDefault();
                                event.stopPropagation();
                                if (isImported) {
                      ...
                      <p className="region-bg-empty">{t("region.bg.emptyCategory")}</p>
                    )}
                  </div>
                </>
              ) : (
                <p className="region-bg-empty">{t("region.bg.emptyLibrary")}</p>
              )}
            </section>
            <div className="region-bg-footer">
              <p className="region-param-message">{selectedBackground ? t("region.bg.hint.canvasSelected") : backgroundMessage}</p>
              <button
                type="button"
              ...
              >
                {saving ? t("region.action.adding") : t("region.action.addBackground")}
              </button>
            </div>
          </> : null}
          ...
          {activePanel === "detection" ? <>
            <div className="region-panel-head">
              <div><h3>{t("region.panel.detection")}</h3></div>
              <button type="button" className="region-panel-close" aria-label={t("common.close")} onClick={() => togglePanel("detection")}>×</button>
            </div>
            <div className="detection-mode-tabs">{(["rect", "learned", "custom"] as const).map((mode) => <button type="button" key={mode} className={detection.mode === mode ? "active" : ""} disabled={readOnly && mode !== detection.mode} onClick={() => {
              if (learningLocked && mode !== "learned") {
                onError(t("region.detection.error.learningLocked"));
                return;
              }
              if (detection.mode === mode) {
                return;
              }
              const next = cloneConfig(regionConfig);
              ...
              // 仅切换配置视图/草稿模式，未下发固件；真正改范围靠「同步」或画点编辑后再标 dirty
              setCurrentConfig({ ...deviceConfig!, regionConfig: next });
            }}>{mode === "rect" ? t("region.detection.mode.rect") : mode === "learned" ? t("region.detection.mode.learned") : t("region.detection.mode.custom")}</button>)}</div>
            {detection.mode === "rect" ? <>
            ...
            }}>{t("region.action.applySync")}</button>
              </div>
            </> : null}
            {detection.mode === "learned" ? <>
              <div className="region-range-switch-row">
                <div>
                  <strong>{t("region.detection.startLearning")}</strong>
                </div>
                <button
                  type="button"
              ...
              {learnedRange?.status === "error" ? (
                <div className="region-detection-footer">
                  ...
                  >
                    {t("region.action.requeryLearn")}
                  </button>
                </div>
              ) : null}
              <p className="region-range-hint">{t("region.detection.learnHint")}</p>
            </> : null}
            {detection.mode === "custom" ? <><div className="custom-range-status">{getDetectionHint(detection, t)}</div><div className="region-panel-actions region-panel-actions-stacked"><div className="region-panel-actions-row"><button type="button" disabled={readOnly || detection.customPointsCm.length === 0} onClick={() => { const next = cloneConfig(regionConfig); next.detection.customPointsCm.pop(); setCurrentConfig({ ...deviceConfig!, regionConfig: next }); setDirty(true); }}>{t("region.action.undo")}</button><button type="button" disabled={readOnly || detection.customPointsCm.length === 0} onClick={() => { const next = cloneConfig(regionConfig); next.detection.customPointsCm = []; next.detection.customConfirmed = false; setCurrentConfig({ ...deviceConfig!, regionConfig: next }); setDirty(true); }}>{t("region.action.clear")}</button></div><div className="region-panel-actions-row"><button type="button" className="primary-button" disabled={readOnly || saving || !canConfirmCustomRange(detection.customPointsCm)} onClick={() => {
              void (async () => {
                const next = cloneConfig(regionConfig);
            ...
            }}>{t("region.action.confirmSync")}</button></div></div></> : null}
          </> : null}
        </aside> : null}
        </div>
        <div className="region-status-bar"><span className="muted">{t("region.status.zoom", { percent: Math.round(cellSize / BASE_CELL * 100) })}</span><span className="muted">|</span><span className="muted">{t("region.status.gridScale")}</span>{readOnly ? <span className="muted">| {t("region.status.offlineReadonly")}</span> : null}</div>
      </div>
    </section>
  );
}
